...
router.get("/productos", async (_req, res, next) => {
    try {
        ...
        res.status(200).sendFile(path.join(path3, "/public/pages/indexPrimerEntrProducts.html"));
    } catch (err) {
        next(err);
    }
});

// router.get("/productoss", async (_req, res, next) => {
//     try {
//         const productosAll = await contenedor.getAll();
//         res.status(200).json({
//             success: true,
...
router.post("/productos", authMiddleware, async (req, res, next) => {
    try {
        const { body } = req;
