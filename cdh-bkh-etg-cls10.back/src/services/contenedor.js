...
class Contenedor {
    ...
    try {
      if (obj.title) {
        ...
        if (dataArchParse.length) {
          ...
          return (`El producto ingresado tiene un id ${idAutomatic}`);
        } else {
          ...
          /* devolvemos al id del objeto creado  */
          return ("El producto ingresado tiene un id es 1");
        }
      } else {
  ...
  /* usamos async ya que es asincrona  */
  async selectTemplater(templeter) {
    /* Manejamos los errores con try catch */
    try {
      switch (templeter) {
        case ("Ejs"):
          fs.writeFile("./selectTemplate.txt", JSON.stringify([{ templater: "ejs" }], null, 2), "utf8");
          return console.log("Ejs");
        case ("Hbs"):
          fs.writeFile("./selectTemplate.txt", JSON.stringify([{ templater: "hbs" }], null, 2), "utf8");
          return console.log("hbs");
        default:
          return ("hbs");
      }
    } catch (err) {
  ...
  /* usamos async ya que es asincrona  */
  async readSelectTemplater() {
    /* Manejamos los errores con try catch */
    try {
      /* leemos  el archivo de this.ruta */
      const dataArch = await fs.readFile("./selectTemplate.txt", "utf8");
      /* lo pasamos a objeto js */
      const dataArchParse = JSON.parse(dataArch);

      if (dataArchParse) {
        /* retornamos el nombre del producto que coincide */
        console.log(`El producto es ${dataArchParse.templater}`);
        return dataArchParse.templater;
      } else {
        /* si no exite el id retornamos un mensaje informandolo */
        console.log("El producto no existe");
        return "no exite el id";
